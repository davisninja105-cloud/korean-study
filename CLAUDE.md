...
- `lib/settings.ts` — Server-side getters/setters for all app settings (`dailyGoalSeconds`, `habitDayStartHour`)
- `lib/habit.ts` — Pure habit-tracking helpers: `computeStreaks`, `computeHabitStats`, `shiftDate`, `habitDateStr`, `formatDuration`
...
- `components/StudySession.tsx` — All three study modes (flashcard, multiple-choice, fill-blank); sentence-aware flashcard with Exposure/Recall sub-mode; sentences rotate across reviews via `(hashStr(id) + reps) % len`; seeds shuffle from card IDs (purity-safe); key prop triggers remount between study-ahead batches
...
- `app/settings/page.tsx` — Dedicated settings page (daily goal, habit day-start hour); add new settings here
...
- `Setting` → generic key/value app settings: `dailyGoalSeconds` (goal in seconds), `habitDayStartHour` (int 0–23, default 2 = 2am)
...
Live at **https://korean-study-five.vercel.app** (Vercel project `jason-d-28-projects/korean-study`), DB on Turso. Redeploy with `npx vercel --prod` (CLI deploy from the local tree — **not** wired to GitHub auto-deploy yet; env vars already set in Vercel). The upload occasionally fails with a transient TLS "bad record mac" — just retry.
...
- **Git:** the entire app (all features) is on `main` but **uncommitted** — there is no commit history beyond the initial Create-Next-App commit. The user has been deferring committing.
