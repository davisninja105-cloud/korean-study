...
- `components/StudySession.tsx` — All three study modes (flashcard, multiple-choice, fill-blank). 3D flip with **dynamic card height** (`useLayoutEffect` measures each face; height and rotation transition simultaneously). Mastery-language grade bar with haptics. Consumes cards **in server order** (foundation-first). Mutable queue (`queue[0]` = current card); REQUEUE_GAP=4; undo restores snapshots. `AudioButton` is mounted next to every example sentence and Korean card front; tap-to-gloss wired via `useWordTap()`.
