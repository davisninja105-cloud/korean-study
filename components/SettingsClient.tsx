...
interface Props {
  ...
  initialRewardColor: string
  initialReadingTextScale: number
  initialReadingAid: boolean
  initialLastAutoSyncedAt: string | null
}
...
export default function SettingsClient({
  ...
  initialButtonColor,
  initialRewardColor,
  initialReadingTextScale,
  initialReadingAid,
  initialLastAutoSyncedAt,
}: Props) {
  // Settings are locally mutable (color pickers, sliders, selects all write
  // back optimistically after a PUT) — useState(initialX) copies, matching
  // CardsClient.tsx's pattern, not HabitsClient's read-only direct-prop-read
  // pattern.
  const [goal, setGoal] = useState(initialGoal)
  const [dayStartHour, setDayStartHour] = useState(initialDayStartHour)
  const [sessionSize, setSessionSize] = useState(initialSessionSize)
  const [buttonColor, setButtonColor] = useState(initialButtonColor)
  const [rewardColor, setRewardColor] = useState(initialRewardColor)
  const [readingTextScale, setReadingTextScale] = useState(initialReadingTextScale)
  const [readingAid, setReadingAid] = useState(initialReadingAid)
  // Read-only after mount (never re-set locally; SyncPanel's onSynced is a
  // no-op) — a direct prop read, not a useState copy.
  const lastAutoSyncedAt = initialLastAutoSyncedAt
  // Theme is client-only (localStorage), never a DB Setting — resolved in a
  // mount effect below, not from server props.
  ...
  // Resolve the client-only theme choice on mount. Promise.resolve().then
  // defers the setState past the synchronous effect body (react-hooks/set-state-in-effect),
  // matching the pattern used for client-local "today" in HabitsClient/WrappedClient.
  useEffect(() => {
