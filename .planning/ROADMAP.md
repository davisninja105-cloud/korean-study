...
- [x] **Phase 35: Service Worker & Offline Review Queue** - Precached app shell plus a persisted review queue that flushes exactly once when the app comes back online (completed 2026-08-10)
...
- [x] 31-07-PLAN.md — gap closure: fix CR-01/CR-02/CR-03 edit-flow defects (Reading Practice stale text, group-bucket relocation, stale sentenceCount badge) + mandatory fail-first-then-green e2e regression spec + WINDOWS.md ledger sync
...
- [x] 32-02-PLAN.md — invariant snapshot module + cross-process sync-triggered invalidation (wave 2)
...
- [x] 32-04-PLAN.md — committed ≤2 proof, tightened perf budgets, no-redeploy freshness e2e (wave 4)
...
- [x] 33-01-PLAN.md — tracer: review write → `dataVersion` Setting row → `GET /api/version` → version-gated `FreshnessWatcher` backstop (`router.refresh()` stays unconditional); plus the `runSync()` bump and the real-DB bump/non-bump regression locks
...
- [x] 34-05-PLAN.md — `FreshnessWatcher` backstop narrowing (blocking decision checkpoint) plus phase-gate e2e
...
| 35. Service Worker & Offline Review Queue | v1.8 | 4/4 | Complete    | 2026-08-10 |
