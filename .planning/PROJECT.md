...
**Shipped:** v1.6 Freshness, Performance & E2E Testing (2026-07-14)

The app is deployed and fully functional. v1.6 diagnosed and fixed a real production staleness bug: an empirical 16-cell route×navigation-path matrix on a production build (`next build && next start`) split the problem into two distinct root causes — back/forward Router Cache reuse and client-shell `useState(initialProps)` non-resync — and both are now fixed via a `FreshnessWatcher` client component (mounted app-wide, fires `router.refresh()` at real staleness boundaries) plus a gated render-phase prop-adoption pattern in `StudyClient`/`CardsClient` that never clobbers an active study session or open editor sheet. A Next.js 16.2.1 reliability flake in `router.refresh()`'s delivery to a mounted client tree was root-caused and closed with a Suspense-independent JSON re-fetch backstop (`useFreshPayload()` context), taking `/study resume` from 0/7 to a clean 5/5 across repeated verification runs. Alongside the fix, an isolated, authenticated Playwright E2E harness now boots a production build against a seeded `file:` SQLite database (structurally prevented from touching production Turso), with a smoke spec, a full freshness-regression suite encoding the diagnosis, a study grade-flow spec (which also caught and fixed a real FSRS bug — `learningSteps` wasn't persisted, so repeatedly-graded cards never graduated out of Learning state), and generous median-of-5 page/API performance budgets. The Playwright MCP server is registered and documented in `CLAUDE.md` for Claude Code to drive the real dev server interactively during exploratory QA. v1.5 (shipped 2026-07-10) audited and hardened the extraction pipeline; v1.4 (shipped 2026-07-05) cleaned up the knowledge graph, made review history durable and browsable, and automated daily sync; v1.3 (shipped 2026-07-03) hardened the two authenticated write routes and refactored `StudySession`; v1.2 (shipped 2026-07-01) eliminated the blank/empty-state flash across every main route; v1.1 (shipped 2026-06-29) completed a systematic UI audit and polish pass — all remain in place underneath this milestone's work.
...
Phase 24 empirically diagnosed the staleness bug as two distinct root causes (Router Cache reuse vs. client-shell non-resync) via a 16-cell production-build matrix. Phase 25 stood up the isolated Playwright E2E harness (seeded `file:` DB, auth setup, smoke spec, red freshness-regression spec encoding the diagnosis). Phase 26 shipped the fix (`FreshnessWatcher` + gated prop adoption), closed a real Next.js reliability flake with a JSON re-fetch backstop after a human-decided approach change, and reached a clean formal acceptance re-run (was 2/5, closed to 5/5). Phase 27 broadened E2E coverage to the study grade-flow (catching a real FSRS bug along the way), added performance budget assertions, and documented the Playwright MCP exploratory-QA workflow. v1.6 is now fully complete: all 16 requirements satisfied.
...
- ✓ Client shells re-sync to fresh server props at real resume/mutation boundaries via `router.refresh()` + gated render-phase prop-sync + a `FreshnessWatcher`, never clobbering an active study session or open editor sheet mid-interaction (FRESH-02) — Phase 26; automated e2e gate coverage (`e2e/freshness-gate.spec.ts`) added in gap closure
- ✓ Returning to Home/Habits/Cards after a study session shows updated stats/due-count/cards immediately, no manual reload (FRESH-03) — Phase 26
...
- ✓ `RootLayout` renders synchronously (no `await` DB read on the render path); a non-httpOnly `ks_settings` cookie mirrors validated button/reward/reading settings and a 3rd pre-paint `<script>` applies them before first paint, so a settings save shows correctly on the very next navigation (LAYOUT-01) — Phase 30
- ✓ Dark-mode skeleton screens (`/study`, `/cards`, `/habits`, `/history`) visibly distinct from the background via an isolated `--skeleton-bg` token; the `/study` lesson-range filter shows a content-shaped skeleton instead of a bare spinner, no layout shift when data lands (PERCEPT-01, PERCEPT-03) — Phase 30
- ✓ PWA cold launch shows no white splash flash — `app/manifest.ts` `background_color`/`theme_color` now match the dark theme (PERCEPT-02) — Phase 30
- ✓ Vercel function region pinned to Turso's primary region via a live `turso db show` lookup, not a guess (REGION-01) — Phase 30
- ✓ `/cards` caps its initial query (cursor pagination via `getCardsPage`/`getCardsGroupCounts`), drops the `sentences` relation from the list read, windows the rendered rows (react-virtuoso), and moves search + lesson filtering server-side; add/edit/delete/swipe-to-delete/tap-to-gloss/group-collapse/Reading Practice all still behave correctly against the paginated list (CARDS-01/CARDS-02/CARDS-03) — Phase 31. Two gap-closure rounds: 31-05/31-06 fixed a mobile sticky-toggle regression and a missing per-card sentence-count signal; 31-07 plus a post-execution code-review/fix cycle closed six edit-flow defects (stale Reading Practice text on save, group-bucket relocation + count bookkeeping, stale sentence-count badge, silent save-failure UX, a phantom-populated-group bug in add/relocate, plus 4 lower-severity API/race hardening fixes) — all with dedicated fail-first-then-green regression coverage, independently re-verified.
- ✓ `/study`'s warm-cache load measured and collapsed from 10–11 physical libSQL round trips to 2 (cold-cache/cache-miss: 3), proven by a committed instrumentation harness (`lib/query-counter.ts` + `tests/study-roundtrips.test.ts`), not asserted by inspection; Phase A folds the live pool + cache-version check into one raw-SQL query, Phase B (confirmed non-duplicative from Phase A, kept rather than merged) folds the sentences relation via `json_group_array`/`json_object`, and the four invariant reads (`CardDependency` edges, known lemmas, sessionSize, lessons) are served from a version-gated `globalThis` snapshot invalidated by any of the three real sync/relink writer processes, proven cross-process with no redeploy (STUDY-01/STUDY-02/STUDY-03) — Phase 32. Session composition proven byte-identical across the rewrite via an ordering diff. A post-execution code review found and fixed 2 CRITICAL cache-invalidation gaps introduced by the new cache (a Settings session-size change not invalidating the warm cache; an empty due/ahead pool permanently pinning a stale cache version) plus 3 warnings, all independently re-verified against real code and real databases before the phase was marked complete.
...
- [ ] v1.8 remaining performance requirements (P3.4–P3.7: freshness-backstop narrowing, local-first IndexedDB shell, service worker + offline queue) — see `.planning/REQUIREMENTS.md`; P3.0/P3.1/P3.5 shipped in Phase 30, P3.2 shipped in Phase 31, P3.3 shipped in Phase 32
...
- `FreshnessWatcher`'s JSON re-fetch backstop has a narrow, self-healing race (no request-sequencing guard against an older same-route response overwriting a newer one) — accepted as non-blocking; add a sequence/timestamp guard if it ever reproduces (Phase 26 code review WR-01)
- v2 E2E requirements tracked but not scheduled: Cards CRUD flow spec (E2E-08), sync-route coverage via a stubbed extraction boundary (E2E-09), WebKit test project if an iOS-Safari-specific bug ever appears (E2E-10)
...
- v1.6 shipped 2026-07-14: 119 commits across 4 phases (14 plans, 31 tasks), 2026-07-10 → 2026-07-13. Diff since v1.5 tag: 147 files, +13454/-8120 lines. New modules: `components/FreshnessWatcher.tsx` (boundary-refresh provider + `useFreshPayload()` JSON backstop context), `e2e/` harness (`playwright.config.ts`, `e2e/helpers/{readers,rsc,resume,mutate,build-guard,test-db}.ts`, `e2e/{fixture,seed,global-setup,auth.setup}.ts`), `e2e/smoke.spec.ts`, `e2e/freshness-{router-cache,client-shell,fresh-paths,gate}.spec.ts`, `e2e/grade-flow.spec.ts`, `e2e/perf.spec.ts`. `scripts/diagnose-freshness.mts` (throwaway diagnosis tool) created in Phase 24, then deleted once superseded by the permanent `e2e/helpers/` ports in Phase 25. Added `playwright`/`@playwright/test` as devDependencies; registered the Playwright MCP server for exploratory QA (documented in `CLAUDE.md`). Also fixed a real production FSRS bug found while writing the grade-flow spec: `CardReview.learningSteps` wasn't persisted, so repeatedly-graded-Good cards looped forever in the Learning state instead of graduating. Closeout: verified_closeout — all phases verified, all 16 requirements complete, no deferred items.
...
| Phase 26's first verification pass (`26-VERIFICATION.md`, 2/5 must-haves) found `/study resume` failing 7/7 live runs — a real, deeply-investigated Next.js 16.2.1 Suspense/Segment-Cache flake, not sloppy work — and was routed to a human decision rather than silently accepted or shipped broken | The user explicitly declined an accepted-risk override and instead locked in the JSON re-fetch alternative already named in `deferred-items.md`'s own recommendation, with a non-negotiable honesty rule (no weakened assertions, no retries-until-green, every run recorded) | ✓ Phase 26 gap closure (Plans 26-04/05/06); re-verification scored 5/5, `/study resume` 5/5 across two independent verification sessions (was 0/7) |
| `FreshnessWatcher` becomes a React context provider (`useFreshPayload`) supplementing `router.refresh()` with a direct client-side JSON fetch of the route's existing API endpoint, rather than replacing the RSC delivery path or retrying it | Six alternative fixes (startTransition, setTimeout-deferred refresh, second coalesced refresh, disabling Nav prefetch, `router.push`/`replace`, a Next.js patch bump) were tried and rejected during investigation; the JSON backstop sidesteps the unreliable Suspense/Segment-Cache application step entirely instead of working around it | ✓ Phase 26-05; adopted through the same gated `prevInitialX`-style render-phase pattern already proven correct, so no new adoption model was invented |
| A narrow, self-healing race in the JSON backstop (no request-sequencing guard against an older same-route response overwriting a newer one) was judged an accepted non-blocking follow-up, not a re-opened gap | Bounded to client-state only, self-heals on the next real boundary event, and did not reproduce across 44+ independent live e2e executions during code review + re-verification; blocking the milestone on a theoretical race with no observed occurrence would trade a proven fix for indefinite delay | ✓ Phase 26 code review (WR-01) + re-verification; tracked as deferred follow-up work, not a Phase 27 blocker |
| Grade-flow E2E spec uses a bounded loop-until-complete keyed off a completion-heading testid, never a fixed grade count | ts-fsrs learning-step intervals make the exact grade count wall-clock/state dependent; a fixed count would be flaky by construction | ✓ Phase 27-01 (E2E-05) |
| A real production FSRS bug (`CardReview.learningSteps` never persisted, so repeatedly-graded-Good cards never graduated out of Learning state) was fixed immediately rather than deferred, once it blocked the grade-flow spec's own acceptance criteria | Genuine, severe correctness defect independent of the test — verified via a standalone ts-fsrs simulation showing the app's actual pre-fix round-trip permanently resets the learning-step index to 0 | ✓ Phase 27-01; `CardReview.learningSteps`/`ReviewLog.learningSteps` added as new `Int @default(0)` columns, threaded through `lib/fsrs.ts`/`lib/dto.ts`/undo restoration with zero API-route changes needed |
...
| `RootLayout` unblocked via a non-httpOnly `ks_settings` cookie mirror (written by `PUT /api/settings`, applied by a 3rd pre-paint `<script>`) instead of an awaited DB read | Removes the only remaining synchronous DB dependency from the render path (LAYOUT-01) while still avoiding a flash of stale settings on the very next navigation | ✓ Phase 30-01; deliberate `httpOnly: false` deviation from the `ks_auth` convention — cookie carries only cosmetic UI preference data, `middleware.ts`'s gate remains the sole access-control mechanism |
| New `--skeleton-bg` token isolated from `--surface-3`/`--surface-1` rather than raising an existing token's dark value | The existing dark `--surface-3` collided visually with `--background`; raising it would have touched 17+ non-skeleton consumers (Nav, Toast, etc.) | ✓ Phase 30-02; dark `--skeleton-bg` reuses the existing `--surface-1` dark shade (#1c2030), no new color invented |
| G-30-2 (production 500 on `/settings`): cookie backfill moved out of the Server Component's render body into a genuine zero-DB `POST /api/settings/backfill-cookie` Route Handler, invoked from `SettingsClient.tsx`'s mount effect | Next.js 16.2.1 only permits `cookies().set()` during the action phase (Server Actions/Route Handlers), not inside a Server Component's render body — the prior CR-01 backfill threw `ReadonlyRequestCookiesError` on every `/settings` visit | ✓ Phase 30-04; permanent regression-guard unit test added forbidding render-body cookie mutation anywhere in `app/settings/page.tsx` |
...
---
*Last updated: 2026-08-08 after Phase 32 (Study Load Round-Trip Collapse)*
