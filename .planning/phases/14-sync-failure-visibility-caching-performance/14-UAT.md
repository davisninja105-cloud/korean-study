...
updated: 2026-07-02T19:35:00Z
---

## Current Test

number: 1
name: SYNC-01 live sync against a deliberately-failing lesson
expected: |
  The SyncPanel failures list shows the lesson's content excerpt (its first non-empty line, ~48 chars, quoted) plus a safe category — e.g. "<first line>…" — extraction failed (will retry on next sync) — NOT 'Lesson 1' and NOT the raw Prisma/extraction error message. The raw error appears only in the server console.
